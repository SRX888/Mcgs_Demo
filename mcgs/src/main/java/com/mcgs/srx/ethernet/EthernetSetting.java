...
public class EthernetSetting extends PreferenceActivity implements Preference.OnPreferenceChangeListener {
  ...
  //  private static final String ETHERNET_USE_STATIC_IP = Settings.IS_ETHERNET_STATUC_OPEN;
    private static final String ETHERNET_USE_STATIC_IP = "IS_ETHERNET_STATUC_OPEN";
...
//        mEthernetManager = (EthernetManager) getSystemService(Context.ETHERNET_SERVICE);
        ...
        Settings.System.putInt(contentResolver,ETHERNET_USE_STATIC_IP, mUseStaticIpCheckBox.isChecked() ? 1 : 0);

        // disable ethernet
        boolean enable = Settings.Secure.getInt(getContentResolver(), "isEnthernetOn", 1) == 1;
        LOG("notify Secure.ETHERNET_ON changed. enable = " + enable);
        if(enable) {
            LOG("first disable");
            Settings.Secure.putInt(getContentResolver(), "isEnthernetOn", 0);
            try {
                Thread.sleep(500);
            } catch (InterruptedException e) {}
            LOG("second enable");
            Settings.Secure.putInt(getContentResolver(), "isEnthernetOn", 1);
        }
    }
